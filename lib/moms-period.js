...
export async function getMomsPeriod({ key } = {}) {
  ...
  const [{ data: settings }, invRes, recRes] = await Promise.all([
    ...
    sb.from("studio_invoices")
      ...
      .gte("paid_at", period.start).lte("paid_at", period.end),
  ...
  // A period is filed when every receipt in it is locked. Nothing else is a reliable
  // signal until the vat_periods table from the architecture doc exists.
