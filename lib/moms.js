 ...
 * The key used to be built as `${field === "total" ? "total" : "vat"}_sek`. Called
 * as sek(inv, "subtotal") — which is how line 99 has always called it — "subtotal"
 * is not "total", so it fell to the other branch and read the NET out of vat_sek.
 * There is no subtotal_sek column, so it could never have resolved correctly.
 *
 * A EUR invoice of net 1 000 / moms 250 filed ruta 05 as 2 750 instead of 11 000
 * and ruta 10 as 0, because the inferred rate came out at 100 % and matched none of
 * 25/12/6. Silently: `unconverted` stayed empty and fileReady stayed true.
 *
 * There are only two stored SEK columns, so the mapping is stated explicitly rather
 * than computed from the field name. The net is DERIVED, never stored — the same
 * way lib/dashboard-data.js:85 has always done it.
 */
const SEK_COLUMN = { total: "total_sek", vat_amount: "vat_sek" };

function sek(row, field) {
  const cur = String(row.currency || "SEK").toUpperCase();
  if (cur === "SEK") {
    const v = row[field];
    /* A genuinely missing figure must not become 0 kr in a tax box. */
    return v == null ? null : Number(v);
  }

  /* The net is total minus VAT. Both legs must be present or we know nothing. */
  if (field === "subtotal") {
    const t = sek(row, "total");
    const v = sek(row, "vat_amount");
    return t == null || v == null ? null : r2(t - v);
  }

  const col = SEK_COLUMN[field];
  if (!col) throw new Error(`sek(): no SEK column for field "${field}"`);
...
export function computeMoms({ invoices = [], receipts = [], period }) {
  ...
  for (const rc of receipts) {
    ...
    // studio_receipts has no `subtotal` column. sek() now derives the net itself
    // and returns null when either leg is missing, so sek(rc,"subtotal") would be
    // safe here — kept explicit because the two legs are needed separately below.
