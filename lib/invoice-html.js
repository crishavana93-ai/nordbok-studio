...
import { vatBreakdown } from "./invoice-compliance";
import { sellerIdentity } from "./seller";

const esc = (s) => String(s ?? "").replace(/[&<>"']/g, (c) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", "\"": "&quot;", "'": "&#39;" }[c]));

const T = {
  sv: {
    invoice: "FAKTURA", credit: "ÄNDRINGSFAKTURA", credit_ref: "Avser faktura", credit_what: "Ändring", number: "Nr", issued: "Datum", due: "Förfaller", ocr: "OCR",
    ...
    taxable_base: "Beskattningsunderlag", vat_rate: "Momssats", vat_in_sek: "Varav moms i svenska kronor", rate_used: "Omräkningskurs",
    f_skatt: "Godkänd för F-skatt", reverse_charge_note: "Omvänd betalningsskyldighet — köparen redovisar moms enligt artikel 196 i mervärdesskattedirektivet.",
  ...
  en: {
    invoice: "INVOICE", credit: "CREDIT NOTE", credit_ref: "Relates to invoice", credit_what: "Change", number: "No.", issued: "Issued", due: "Due", ocr: "Reference",
    ...
    taxable_base: "Taxable amount", vat_rate: "VAT rate", vat_in_sek: "VAT in Swedish kronor", rate_used: "Exchange rate",
...
export function renderInvoiceHTML({ invoice, client, settings, items, venture, creditOf }) {
  const lang = invoice.language === "en" ? "en" : "sv";
  const t = T[lang];
  const ccy = invoice.currency || "SEK";
  const f = (n) => fmtMoney(n, ccy);
  /* The seller block is the field a reviewing bookkeeper checks first. lib/seller.js
     decides whether the venture may head the invoice or must sit under it as a brand
     reference -- never both, never guessed here. */
  const seller = sellerIdentity({ settings, venture, lang });
  ...
  // Prefer the breakdown frozen at send time so a historical invoice always
  // reproduces exactly as it was sent; fall back to recomputing for drafts.
  const bd = Array.isArray(invoice.vat_breakdown) ? { rows: invoice.vat_breakdown } : vatBreakdown(items || []);
...
<div class="head">
  <div>
    <div class="biz">${esc(seller.headerName)}</div>
    ${seller.subLine ? `<div style="color:#525258;font-size:12.5px;margin-top:2px">${esc(seller.subLine)}</div>` : ""}
    ...
    <div style="color:#525258;margin-top:8px">${lang === "sv" ? "Org.nr" : "Company ID"}: ${esc(settings?.org_nr || settings?.personnummer || "—")}${settings?.vat_number ? `<br>${lang === "sv" ? "Moms-nr" : "VAT no."}: ${esc(settings.vat_number)}` : ""}</div>
    ${settings?.f_skatt_approved ? `<div style="margin-top:8px"><span class="stamp">${t.f_skatt}</span></div>` : ""}
  </div>
  <div>
    <h1>${isCredit ? t.credit : t.invoice}</h1>
    <div class="meta">
    ...
    ${seller.brandLine ? `<div style="margin-top:8px;color:#525258;font-size:12.5px">${esc(seller.brandLine)}</div>` : ""}
    ${isCredit ? `<div style="margin-top:10px;padding:8px 10px;border:1px solid #0d3a2a;border-radius:6px;font-size:12.5px;text-align:left">
      <strong>${t.credit_ref}: ${esc(creditOf?.invoice_number || "—")}</strong>
      ${creditOf?.issue_date ? `<br><span style="color:#525258">${fmtDate(creditOf.issue_date, t.date_locale)}</span>` : ""}
      ${invoice.credit_reason ? `<br><span style="color:#525258">${t.credit_what}: ${esc(invoice.credit_reason)}</span>` : ""}
    </div>` : ""}
  </div>
</div>
...
<div class="totals">
  <table>
    <tbody>
      ${showBreakdown
        ? rateRows.map((r) => `
            <tr><td>${t.taxable_base} ${esc(r.rate)}%</td><td class="num">${f(r.net)}</td></tr>
            <tr><td style="color:#8a8a90">${t.vat_amount} ${esc(r.rate)}%</td><td class="num" style="color:#8a8a90">${f(r.vat)}</td></tr>`).join("")
      ...
      ${ccy !== "SEK" && invoice.doc_vat_sek != null ? `
      <tr><td colspan="2" style="padding-top:10px;border-top:1px solid #e7e7e0">
        <div style="font-size:12px;color:#525258">
          <strong style="color:#111">${t.vat_in_sek}: ${fmtMoney(invoice.doc_vat_sek, "SEK")}</strong><br>
          ${t.rate_used}: 1 ${esc(ccy)} = ${Number(invoice.doc_fx_rate).toFixed(4)} SEK
          ${invoice.doc_fx_date ? ` (${esc(String(invoice.doc_fx_source || "ECB").toUpperCase())}, ${fmtDate(invoice.doc_fx_date, t.date_locale)})` : ""}
        </div>
      </td></tr>` : ""}
    </tbody>
  </table>
...
<div class="footer">
  ...
  <div style="text-align:right">
    ...
    ${t.interest_note}<br>
    ${seller.footer ? esc(seller.footer) : ""}
  </div>
</div>
