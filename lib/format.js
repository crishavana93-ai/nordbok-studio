...
/* ISO code → what a Swedish reader expects to see. */
const UNIT = { SEK: "kr", sek: "kr" };
...
export function money(value, opts = {}) {
  ...
  /* Callers pass an ISO code because that is what the database stores. Swedish
   * convention writes the domestic currency as "kr", never "SEK" — "1 234,50 kr".
   * Foreign amounts keep their code, which is unambiguous in bookkeeping.
   * Mapping here rather than at each call site is the point of having one formatter. */
  const unit = UNIT[opts.currency] ?? opts.currency ?? "kr";
  const negative = n < 0;
...
export function spoken(value, opts = {}) {
  ...
  const decimals = opts.decimals ?? 2;
  const unit = UNIT[opts.unit] ?? opts.unit ?? "kr";
