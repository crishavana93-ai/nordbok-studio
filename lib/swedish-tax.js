...
export function suggestBasAccount(vendor = "", description = "") {
  ...
  /* No keyword matched. Returning Förbrukningsinventarier here was a defect:
     it labelled every unrecognised purchase as consumable equipment, which is
     a specific claim that happens to be false. 6991 says "uncategorised cost",
     which is true, and an accountant can see it needs attention. */
  return { account: "6991", label: "Övriga externa kostnader, avdragsgilla", ne: "R5", keywords: [] };
}
...
/** @deprecated Räknade fel på tre sätt; se lib/skatt-2026.js. Kvar bara för att
 *  gamla anrop inte ska krascha — den delegerar och översätter fältnamnen. */
export function estimateTax(netProfitSek, opts = {}) {
  const r = beraknaSkatt(netProfitSek, {
    kommunalskatt: opts.kommunalskatt,
    reduceradAvgift: opts.reduceradAvgift,
  });
  return {
    egenavgifter: r.egenavgifter,
    schablonavdrag: r.schablonavdrag,
    kommunalskatt: r.kommunalskatt,
    statligskatt: r.statligskatt,
    total_tax: r.total_skatt,
    taxable_income: r.beskattningsbar_inkomst,
    nedsattning: r.nedsattning,
    grundavdrag: r.grundavdrag,
    andel_av_overskott: r.andel_av_overskott,
